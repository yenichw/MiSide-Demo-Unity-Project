...
Shader "Hidden/Colorful/Noise"
{
    ...
    CGINCLUDE
        ...
        float4 frag_mono(v2f_img i) : SV_Target
        {
            float4 color = tex2D(_MainTex, StereoScreenSpaceUVAdjust(i.uv, _MainTex_ST));
            float n = simpleNoise(i.uv + _Params.x) * 2.0;
            return lerp(color, color * n, _Params.y);
        }

        float4 frag_colored(v2f_img i) : SV_Target
        {
            float4 color = tex2D(_MainTex, StereoScreenSpaceUVAdjust(i.uv, _MainTex_ST));
            ...
            return lerp(color, color * float4(nr, ng, nb, na), _Params.y);
        }

        float4 frag_mono_lum(v2f_img i) : SV_Target
        {
            float4 color = tex2D(_MainTex, StereoScreenSpaceUVAdjust(i.uv, _MainTex_ST));
            float n = simpleNoise(i.uv + _Params.x) * 2.0;
        ...
        float4 frag_colored_lum(v2f_img i) : SV_Target
        {
            float4 color = tex2D(_MainTex, StereoScreenSpaceUVAdjust(i.uv, _MainTex_ST));
            float n = simpleNoise_fracLess(i.uv + _Params.x);
            float nr = frac(n) * 2.0;
            ...
            float na = frac(n * 1.3647) * 2.0;
            float lum = luminance(color.rgb);
            return lerp(color, color * float4(nr, ng, nb, na), _Params.y * (1.0 - lerp(0.0, lum, _Params.z)));
        }
